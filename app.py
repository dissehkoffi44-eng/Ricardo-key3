...
# CSS THÈME : CLAIR & LISIBLE (Style Studio Moderne)
st.markdown("""
    <style>
    .stApp { background-color: #F4F7F6; color: #1E1E1E; }
    h1 { font-family: 'Helvetica Neue', sans-serif; color: #2C3E50; text-align: center; font-weight: 800; padding: 20px; }
    .stMetric { 
    ...
    .stExpander { border: 1px solid #D1D1D1 !important; background-color: #FFFFFF !important; border-radius: 10px !important; }
    div[data-testid="stMarkdownContainer"] p { font-size: 1.1rem; color: #34495E; }
    </style>
...
def get_camelot_pro(key, mode):
    # Règle personnalisée demandée : F# Minor = 11A
...
def calculate_energy(y, sr):
    ...
    energy_score = (rms * 25) + (rolloff / 1100) + (float(tempo) / 160)
...
def analyze_ultra_precision(y, sr):
    ...
    chroma = librosa.feature.chroma_cqt(y=y_harm, sr=sr, bins_per_octave=24, tuning=tuning, fmin=librosa.note_to_hz('C2'))
...
st.markdown("<p style='text-align: center;'>Analyseur harmonique de studio haute fidélité</p>", unsafe_allow_html=True)

files = st.file_uploader("", type=['mp3', 'wav', 'flac'], accept_multiple_files=True)

if files:
    for file in files:
        with st.expander(f"📂 FICHIER : {file.name}", expanded=True):
            with st.spinner("Analyse spectrale en cours..."):
                ...
                for start_t in range(0, int(duration) - 15, 10):
                    ...
                    if score > 0.45:
                        ...
                        timeline_data.append({"Temps": start_t, "Note": f"{key}{m_label}", "Mode": mode, "Confiance": score})
                ...
                if votes:
                    ...
                    c1.metric("CLÉ & MODE", f"{f_key} {f_mode.upper()}")
                    c2.metric("CAMELOT", f_camelot)
                    c3.metric("ÉNERGIE", f"{energy}/10")
                    c4.metric("BPM", f"{int(float(tempo))}")

                    # --- GRAPHIQUE ---
                    st.markdown("### 📊 Stabilité du morceau")
                    ...
                    fig.update_layout(plot_bgcolor='white', paper_bgcolor='white', font_color='#1E1E1E')
...
for item in st.session_state.history:
    ...
    <div class="history-card">
        <b>{item['Nom']}</b> | <span style="color:#D4AF37">{item['Cle']} ({item['Camelot']})</span> | 
